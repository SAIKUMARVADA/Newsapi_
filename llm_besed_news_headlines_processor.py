...
API_KEY = "3c8f25bf3f0945aeb14b3cffa2d007ff"
HEADLINE_COUNT =3  
ARTICLE_COUNT = 5
...
def generate_headlines(prompt):
    news_headline_tokens = tokenizer(prompt, return_tendsors="pt").input_ids
...
def process_news_articles(news_articles):
    ...
    for id, news_article in enumerate(news_articles):
        ...
        description = news_article.get("description") or ""
         ...
         # Newa Article = "title + description + content"     
       ...
       # Prepare prompt : Get Healine for Each Article (You are in for loop)
        ...
        # Get the Original Prompt Embudding(prompt)
        news_text_embudding = get_mean_embedding(prompt)

        result = []
        for headline in headline_candidates:
            headline_mean_embudding = get_mean_embedding(headline_candidates)
